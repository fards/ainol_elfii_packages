...
public class ModePicker extends RelativeLayout implements View.OnClickListener,
	...
	//panorama can just work with back camera, so when we didn't hava back front, disable it.
	public static boolean ChkIfNeedPanorama() {
		int cameranum = android.hardware.Camera.getNumberOfCameras();
		CameraInfo cinfo = new CameraInfo();
		boolean bFindBackCamera = false;
    ...
    public ModePicker(Context context, AttributeSet attrs) {
		...
		if(ChkIfNeedPanorama() == false) {
			MODE_NUM = 2;
			mEnablePanorama = false;
		} else {
			MODE_NUM = 3;
			mEnablePanorama = true;
		}
    ...
    protected void onFinishInflate() {
		...
		if( mEnablePanorama == true) {
			mModeSelectionIcon[MODE_PANORAMA] = (RotateImageView) findViewById(R.id.mode_panorama);
		} else {
			View modeview = findViewById(R.id.mode_panorama);//if exist ,remove it
			if(modeview != null) {
				ViewGroup viewparent = (ViewGroup)modeview.getParent();
				viewparent.removeView(modeview);
			}
		}
        ...
        if (mCurrentModeFrame != null) {
			...
			if( mEnablePanorama == true) {
				...
				View modeview = findViewById(R.id.mode_2);//if exist ,remove it
				if(modeview != null) {
					ViewGroup viewparent = (ViewGroup)modeview.getParent();
					viewparent.removeView(modeview);
			}
    ...
    public void onClick(View view) {
        if(m_Enable == false) {
            Log.d(TAG,"modepicker is not enabled");
            return;
        }
