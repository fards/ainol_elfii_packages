...
public class IndicatorControlWheel extends IndicatorControl implements
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
		...
		if(IndicatorControlWheel.this.getResources().getBoolean(R.bool.config_wheel_enable)){
        	dx = event.getX() - mCenterX;
        	dy = mCenterY - event.getY();
			radius = Math.sqrt(dx * dx + dy * dy);
			}
		else{
        	dx = event.getX() - mCenterX +35;
        	dy = mCenterY - event.getY()+10;
			radius = Math.sqrt(dx * dx + dy * dy);
			}
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (selectedIndex >= 1) {
			...
			if(IndicatorControlWheel.this.getResources().getBoolean(R.bool.config_wheel_enable)){
            	innerR = (float) mShutterButtonRadius;
            	outerR = (float) (mShutterButtonRadius + mStrokeWidth +
                    EDGE_STROKE_WIDTH * 0.5);
				}
			else{
            	innerR = (float) mShutterButtonRadius-18;
				outerR = (float) (mShutterButtonRadius + mStrokeWidth +
