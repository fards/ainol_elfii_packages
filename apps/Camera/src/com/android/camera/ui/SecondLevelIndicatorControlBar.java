...
public class SecondLevelIndicatorControlBar extends IndicatorControl implements
    ...
    private int getTouchViewIndexOnPort(int x, int width) {
    ...
    private int getTouchViewIndexOnLand(int y, int height) {
        // If the current touch location is on close icon and above.
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
		...
		if(mContext.getResources().getBoolean(R.bool.TabletUsePhoneUI)==true) {
			...
			index = getTouchViewIndexOnLand((int) y, height);
		}
		else {
			...
			if (x > width) x = width;
			index = getTouchViewIndexOnPort((int) x, width);
		}
    ...
    protected void onLayout(
		...
		if(mContext.getResources().getBoolean(R.bool.TabletUsePhoneUI)==true) {
			...
			int offsetY = 0;
			increment = iconHeight + ICON_SPACING;

		    // left top right bottom
			// The first icon is close button.
			offsetY = padding;
			mCloseIcon.layout(0, offsetY, width, (offsetY + iconHeight));

			// And layout the divider line.
			offsetY = iconHeight + 2 * padding;
			mDivider.layout( padding, offsetY, (width - padding),
				(offsetY + mDivider.getMeasuredHeight()));

			offsetY = height - (count - mNonIndicatorButtonCount)*increment - padding ;
			for (int i = mNonIndicatorButtonCount; i < count; i++) {
				getChildAt(i).layout(0, offsetY, width, offsetY + iconHeight);
				offsetY += increment;
			}
		}
		else {
			...
			int offsetX = padding;
			increment = iconWidth + ICON_SPACING;
