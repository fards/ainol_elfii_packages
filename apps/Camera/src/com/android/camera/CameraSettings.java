...
public class CameraSettings {
    ...
    private void initPreference(PreferenceGroup group) {
        ...
        IconListPreference antibanding =
                (IconListPreference) group.findPreference(KEY_ANTI_BANDING);
		...
		if(antibanding != null) buildAntiBandingList(group,antibanding);
    }
	...
	private void buildAntiBandingList(
            PreferenceGroup group, IconListPreference preference) {
		List<String> vals = mParameters.getSupportedAntibanding();
		int size = vals.size();
		Log.d(TAG,"buildAntiBandingList "+String.valueOf(size));
		CharSequence[] entryValues = new CharSequence[size];
		for (int i = 0; i < size; i++) {
			Log.d(TAG,"buildAntiBandingList add "+vals.get(i));
			entryValues[i] = vals.get(i);
		}
