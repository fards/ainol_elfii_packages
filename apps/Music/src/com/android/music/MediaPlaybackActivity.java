    ...
    View.OnTouchListener, View.OnLongClickListener
{
	...
	public static final String LRCPATH = "/sdcard/external_sdcard/";
	public static final String LRCPATH_NAND = "/sdcard/";
    ...
    public void onCreate(Bundle icicle)
    {
		...
		mTelephony =  (TelephonyManager) this.getSystemService(Context.TELEPHONY_SERVICE);
	...
	private File findLrc(String path,String fileName,String trackName){
			...
			if (file.isDirectory()) {
				...
				for (int i = 0; i < f.length; i++) {
					...
					ext = f[i].getName().substring(
						f[i].getName().lastIndexOf(".") < 0 ? 0 : f[i]
								.getName().lastIndexOf("."),
							f[i].getName().length());
					...
					if (ext.equals(".lrc")) {
						...
						Log.d("MediaPlaybackActivity", "mp3 LRC filename = "
								+ f[i].getName());

					if (f[i].getName().contains(fileName)||f[i].getName().contains(trackName)) {

							lrc = f[i];
						}
						
//						 if (f[i].getName()
//						 .substring(0, f[i].getName().indexOf("."))
//							.equals(artistName + "-" + trackName)) {
//						
//						 lrc = f[i];
//						 break;
//						 }
	...
	private void initLrc() {
		...
		try {
			...
			String mFilePath = mService.getPath();
			...
			Log.d("MediaPlaybackActivity","FilePath = "+mFilePath);		
			Log.d("MediaPlaybackActivity","mFileFullName = "+mFileFullName);
			Log.d("MediaPlaybackActivity", "TrackName = " + mTrackName);
			Log.d("MediaPlaybackActivity", "ArtistName = " + mArtistName);
			
			mFileName = mFileFullName.substring(0,mFileFullName.lastIndexOf("."));
			Log.d("MediaPlaybackActivity", "mFileName = " + mFileName);
			String path = LRCPATH;
			File lrc = null;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (MusicUtils.getCurrentAudioId() >= 0) {
			...
			if(mTelephony != null && mTelephony.isVoiceCapable()){
            ...
            if (getPackageManager().resolveActivity(i, 0) != null && SystemProperties.getBoolean("ro.audioEffectEnabled", false)) {
            ...
            switch (msg.what) {
				...
				try {
					long position = mPosOverride < 0 ? mService.position()
							: mPosOverride;
					// Log.d("MediaPlaybackActivity", "position = " + position);
