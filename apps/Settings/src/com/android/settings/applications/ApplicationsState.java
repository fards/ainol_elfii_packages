...
public class ApplicationsState {
    ...
    void resume(Callbacks callbacks) {
        ...
        synchronized (mEntriesMap) {
            ...
            //Do not display some APKs with Phone, when there is no phone( dgt edited)
            if(!Utils.isVoiceCapable(mContext)){
                List<ApplicationInfo> delList = new ArrayList<ApplicationInfo>();
                for (ApplicationInfo appInfo : mApplications) {
                    if (appInfo.packageName.equalsIgnoreCase("com.android.phone") 
                        || appInfo.packageName.equalsIgnoreCase("com.android.providers.telephony")
                        || appInfo.packageName.equalsIgnoreCase("com.android.contacts")
                        ||appInfo.packageName.equalsIgnoreCase("com.android.providers.contacts")) {
                        delList.add(appInfo);                
                    }            
                }           
                mApplications.removeAll(delList);
            }
