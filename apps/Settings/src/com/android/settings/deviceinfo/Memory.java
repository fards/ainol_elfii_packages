...
public class Memory extends SettingsPreferenceFragment {
    ...
    private static final int DLG_CONFIRM_USB_UNMOUNT = 3;
    private static final int DLG_ERROR_USB_UNMOUNT = 4;
    ...
    public void onCreate(Bundle icicle) {
        ...
        mStorageVolumePreferenceCategories = new StorageVolumePreferenceCategory[length + 1];
        ...
        if(mStorageManager.getVolumeState("/mnt/sda").equals(Environment.MEDIA_MOUNTED))
            storageVolume = new StorageVolume("/mnt/sda","USB storage",true,true,0,true,0);
        else
            storageVolume = new StorageVolume("/mnt/sda1","USB storage",true,true,0,true,0);
            
        mStorageVolumePreferenceCategories[length] = new StorageVolumePreferenceCategory(
             getActivity(), mResources, storageVolume, mStorageManager, false);
        getPreferenceScreen().addPreference(mStorageVolumePreferenceCategories[length]);
        mStorageVolumePreferenceCategories[length].init();
    ...
    StorageEventListener mStorageListener = new StorageEventListener() {
        ...
        public void onStorageStateChanged(String path, String oldState, String newState) {
            ...
            if(path.equals("/mnt/sda")){  
                StorageVolumePreferenceCategory svpc = mStorageVolumePreferenceCategories[mStorageVolumePreferenceCategories.length -1];                                
                svpc.getStorageVolume().setPath("/mnt/sda");
                svpc.onStorageStateChanged();              
            }else if(path.equals("/mnt/sda1")){         
                StorageVolumePreferenceCategory svpc = mStorageVolumePreferenceCategories[mStorageVolumePreferenceCategories.length -1];                                         
                svpc.getStorageVolume().setPath("/mnt/sda1");
                svpc.onStorageStateChanged();
            }
            else{
                for (int i = 0; i < mStorageVolumePreferenceCategories.length; i++) {
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        for (int i = 0; i < mStorageVolumePreferenceCategories.length; i++) {
            ...
            if (svpc.mountToggleClicked(preference)) {
                ...
                String device = storageVolume != null
    ...
    public Dialog onCreateDialog(int id) {
        ...
        case DLG_ERROR_UNMOUNT:
            ...
            .create();
         case DLG_CONFIRM_USB_UNMOUNT :
                return new AlertDialog.Builder(getActivity())
                    .setTitle(R.string.dlg_confirm_unmount_usb_title)
                    .setPositiveButton(R.string.dlg_ok, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            doUnmount("USB storage");
                        }})
                    .setNegativeButton(R.string.cancel, null)
                    .setMessage(R.string.dlg_confirm_unmount_usb_text)
                    .create();
        case DLG_ERROR_USB_UNMOUNT:
                return new AlertDialog.Builder(getActivity())
            .setTitle(R.string.dlg_error_unmount_usb_title)
            .setNeutralButton(R.string.dlg_ok, null)
            .setMessage(R.string.dlg_error_unmount_usb_text)
            .create();               
        }
        return null;
    }

    private void doUnmount(String device) {
        // Present a toast here
        if(device.equals("USB storage")){
        ...
        try {
            ...
            if(device.equals("USB storage")){               
                showDialogInner(DLG_ERROR_USB_UNMOUNT);
            }else{
    ...
    private void unmount(String device) {
        // Check if external media is in use.
        try {
           if (hasAppsAccessingStorage()) {
               // Present dialog to user
               if(device.equals("USB storage")){
                    showDialogInner(DLG_CONFIRM_USB_UNMOUNT);
               }else{
